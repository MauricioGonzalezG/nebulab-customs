...
const BRAND_URL = 'https://nebulab3d.com.co';
const BRAND_PRIMARY = '#8b5cf6'; // Violet
const BRAND_ACCENT = '#06b6d4'; // Cyan
 ...
 */
const getItemDescriptionHtml = (item: CartItem): string => {
...
<body style="margin: 0; padding: 0; background-color: ${BRAND_DARK};">
  <!-- Preheader text hidden in inbox -->
  ...
  const contentHtml = `
    ...
    <div style="text-align: center; margin-bottom: 24px;">
      ...
      <h1 style="margin: 16px 0 6px 0; font-size: 24px; font-weight: 800; color: #ffffff; letter-spacing: -0.5px;">
        ¡Gracias por tu compra, ${shipping.fullName.split(' ')[0]}!
      </h1>
    ...
    <div style="background-color: #0f172a; border-radius: 14px; padding: 18px; margin-bottom: 28px; border: 1px solid #1e293b;">
      ...
      <div style="font-size: 14px; color: #f1f5f9; line-height: 1.5;">
        ...
        <span style="color: #94a3b8;">Teléfono:</span> ${shipping.phone} • <span style="color: #94a3b8;">Email:</span> ${shipping.email}
      </div>
    </div>
...
/**
 * 2. Admin New Order Alert Template
 */
export const buildAdminNewOrderEmail = (order: Order): { subject: string; html: string } => {
  ...
  const contentHtml = `
    ...
    <div style="background-color: #0f172a; border-radius: 14px; padding: 18px; margin-bottom: 20px; border: 1px solid #1e293b;">
      ...
      <div style="font-size: 14px; color: #f1f5f9; line-height: 1.6;">
        <strong>Nombre:</strong> ${shipping.fullName}<br/>
  ...
  return {
    subject: `🚨 [Nueva Venta] Orden #${order.id} (${formattedTotal}) - ${shipping.fullName}`,
    html: wrapInEmailTemplate('Nueva Venta Registrada', `Nueva orden #${order.id} de ${shipping.fullName} por ${formattedTotal}`, contentHtml),
  };
};

/**
 * 3. Customer Status Change Template
 */
export const buildStatusChangeEmail = (order: Order, newStatus: Order['status']): { subject: string; html: string } => {
  const statusLabels: Record<Order['status'], { title: string; badge: string; color: string; desc: string }> = {
    confirmed: {
